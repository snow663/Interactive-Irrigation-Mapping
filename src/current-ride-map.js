...
const SELECTED_RIDE_KEY = 'interactive-irrigation-selected-ride';
...
const ui = {
  menu: $('rideMenu'),
  menuToggle: $('rideMenuToggle'),
  cards: $('rideCards'),
  reload: $('reloadRideDataBtn'),
  title: $('rideTitle'),
  subtitle: $('rideSubtitle'),
  stats: $('rideStats'),
  gps: $('gpsRideBtn'),
  follow: $('followRideBtn'),
  fit: $('fitRideBtn'),
  clearLive: $('clearLiveTrackBtn')
};

let state = { zones: [], drawnTrails: [], assets: [], logs: [] };
let selectedRideId = localStorage.getItem(SELECTED_RIDE_KEY) || '';
let watchId = null;
let follow = true;
let liveTrack = [];
...
function rideColor(ride, index = 0) { return ride.color || rideTracks(ride.id).find((trail) => trail.color)?.color || DEFAULT_RIDE_COLORS[index % DEFAULT_RIDE_COLORS.length]; }
function rideRider(ride) { return ride.ditchRider || ride.rider || rideTracks(ride.id).find((trail) => trail.ditchRider)?.ditchRider || ''; }
...
function renderRideMenu() {
  ...
  rides.forEach((ride, index) => {
    ...
    button.innerHTML = `<strong><span class="ride-color-dot"></span>${escapeHtml(ride.name)}</strong><span>${rider ? `Ditch rider: ${escapeHtml(rider)} • ` : ''}${tracks.length} track${tracks.length === 1 ? '' : 's'} • ${markers.length} marker${markers.length === 1 ? '' : 's'} • ${logs.length} log${logs.length === 1 ? '' : 's'}</span>`;
...
function rideBounds(ride) {
  ...
  return points.length ? L.latLngBounds(points.map(toLatLng)) : null;
}
function fitSelectedRide() {
  const ride = selectedRide();
  const bounds = ride ? rideBounds(ride) : null;
  if (bounds?.isValid()) map.fitBounds(bounds, { paddingTopLeft: [160, 60], paddingBottomRight: [80, 120], maxZoom: 16 });
}
function setSelectedRideBounds() {
  const ride = selectedRide();
  const bounds = ride ? rideBounds(ride) : null;
  if (bounds?.isValid()) map.setMaxBounds(bounds.pad(0.75));
  else map.setMaxBounds(null);
}
function selectRide(rideId, { fit = false, closeMenu = false } = {}) {
  selectedRideId = rideId;
  localStorage.setItem(SELECTED_RIDE_KEY, rideId || '');
  renderRideMenu();
  drawSelectedRide();
...
function drawSelectedRide() {
  ...
  if (ride.boundary.length >= 3) {
    L.polygon(ride.boundary.map(toLatLng), { color, weight: 4, fillOpacity: 0.05, dashArray: '10 6' }).bindPopup(`<strong>${escapeHtml(ride.name)}</strong><br>${rider ? `Ditch rider: ${escapeHtml(rider)}<br>` : ''}${escapeHtml(ride.notes || '')}`).addTo(boundaryLayer);
  }
  for (const trail of tracks) {
    const lineColor = trail.color || color;
    L.polyline(trail.points.map(toLatLng), { color: lineColor, weight: trail.featureType === 'ride-track' ? 7 : 5, opacity: 0.92 }).bindPopup(`<strong>${escapeHtml(trail.name)}</strong><br>${rider ? `Ditch rider: ${escapeHtml(rider)}<br>` : ''}${escapeHtml(trail.notes || '')}`).addTo(trackLayer);
  }
  for (const marker of markers) {
    const label = L.divIcon({ className: 'ride-marker-label', html: escapeHtml(marker.name || marker.type || 'Marker') });
    L.marker([marker.lat, marker.lng], { icon: label }).bindPopup(`<strong>${escapeHtml(marker.name)}</strong><br>${escapeHtml(marker.type)}<br>${escapeHtml(marker.notes || '')}`).addTo(markerLayer);
  }
...
function gpsError(error) {
  ui.subtitle.textContent = { 1: 'Location permission denied.', 2: 'Location unavailable.', 3: 'GPS timed out.' }[error.code] || error.message || 'GPS error.';
}
function startGps() {
